...
function FinancialView({ go }) {
  ...
  // Commission records — use originalAmount (native currency) as primary, c.amount as USD fallback
  const commissions = useCommissions();
  // Per-currency totals using originalAmount (the actual recorded native amount)
  const commByCur = {};
  commissions.forEach(function(c) {
    const cur = c.currency || "USD";
    if (!commByCur[cur]) commByCur[cur] = { count: 0, origTotal: 0, invoiceTotal: 0, usdTotal: 0 };
    commByCur[cur].count++;
    // originalAmount = commission in native currency; invoiceAmount = full invoice in native currency
    commByCur[cur].origTotal += +c.originalAmount || +c.amount || 0;
    commByCur[cur].invoiceTotal += +c.invoiceAmount || 0;
    commByCur[cur].usdTotal += +c.amount || 0;
  });
  // USD total: sum of stored USD amounts across all records
  ...
  return (
    <div className="cb-grid" style={{ gap: "var(--gap-grid)" }}>
      ...
      {/* Currency breakdown — one permanent card per currency, always visible */}
      <div className="cb-grid" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: "var(--gap-grid)" }}>
        {["USD","CAD","EUR","INR","TRY"].map(function(cur) {
          const info = FD_CUR[cur];
          const data = commByCur[cur] || { count: 0, origTotal: 0, invoiceTotal: 0, usdTotal: 0 };
          ...
          return (
            <div key={cur} className="cb-card" style={{ padding: "var(--pad-card)", display: "flex", flexDirection: "column", gap: 10, opacity: hasData ? 1 : 0.55 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 600, background: hasData ? "var(--sky-100)" : "var(--bg-muted)", color: hasData ? "var(--sky-700)" : "var(--text-faint)", borderRadius: 99, padding: "2px 8px" }}>{data.count} rec</span>
              </div>
              <div>
                <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 2 }}>Commission</div>
                <div style={{ fontSize: 22, fontWeight: 800, color: hasData ? "var(--teal-700)" : "var(--text-muted)", fontFamily: "var(--font-display)", lineHeight: 1 }}>{origVal}</div>
                {cur !== "USD" && hasData ? <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 3 }}>≈ {fmtMoney(data.usdTotal)} USD</div> : null}
              </div>
              {invVal ? (
