...
(function () {
  ...
  window.CBStore = {
    ...
    addCommission: function (input) {
      var c = { id: rid("HC"), hospital: input.hospital || "", patient: input.patient || "", amount: +input.amount || 0, currency: input.currency || "USD", originalAmount: input.originalAmount != null ? +input.originalAmount : (+input.amount || 0), commissionRate: input.commissionRate || null, status: input.status || "Unpaid", dueDate: input.dueDate || "", dueISO: input.dueISO || "", recorded: input.recorded || today(), recordedISO: input.recordedISO || "", notes: input.notes || "" };
