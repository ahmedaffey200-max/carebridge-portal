/* ============================================================
   ...
   ============================================================ */
(function () {
  var KEY = "cb_portal_state_v10";
  // NOTE: never bump this key — data must persist across updates.
  ...
  function seedTravel(p) {
    p = p || { stage: 0 };
    var stage = p.stage || 0;
    var visa = stage >= 6 ? "Approved" : stage >= 4 ? "Submitted" : stage >= 1 ? "Under Review" : "Not Started";
    var flight = stage >= 6 ? "Completed" : stage >= 5 ? "In Progress" : stage >= 4 ? "Booked" : "Not Booked";
    var hotel = stage >= 7 ? "Checked In" : stage >= 5 ? "Reserved" : stage >= 4 ? "Waiting Confirmation" : "Not Reserved";
    var pickup = stage >= 7 ? "Completed" : stage >= 6 ? "Driver Assigned" : stage >= 5 ? "Scheduled" : "Not Scheduled";
    ...
    return {
      ...
      hotel: { status: hotel, name: stage >= 4 ? "Partner guest residence" : "", checkIn: stage >= 5 ? daysAgo2(2) : "", checkOut: stage >= 5 ? aheadDate(12) : "", nights: stage >= 5 ? 14 : 0, note: "" },
      pickup: { status: pickup, driver: stage >= 6 ? "Ahmed (Carebridge transfer)" : "", note: "" },
      logistics: { "Visa Submitted": stage >= 4, "Visa Approved": stage >= 6, "Flight Booked": stage >= 4, "Flight In Progress": stage >= 5, "Hotel Booked": stage >= 5, "Hotel Waiting": stage === 4, "Airport Waiting": stage === 6, "Packed": stage >= 5, "Arrived": stage >= 6, "Completed": stage >= 8 },
      ...
      review: { coordination: stage >= 6 ? "Approved" : "In Review", document: stage >= 6 ? "Approved" : "Pending", financial: stage >= 7 ? "Approved" : "Pending", finalApproval: stage >= 8 ? "Approved" : "Pending" },
      returnFlight: { airline: stage >= 8 ? "Turkish Airlines" : "", flightNo: stage >= 8 ? "TK605" : "", date: stage >= 8 ? aheadDate(14) : "", time: stage >= 8 ? "14:30" : "", destination: "Mogadishu (MGQ)", status: stage >= 8 ? "Booked" : "Not Booked" },
    };
  }
