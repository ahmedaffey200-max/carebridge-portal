...
(function () {
  ...
  // Always sync role from the login session (cb_role set by Carebridge Login.html)
  var _loginRole = localStorage.getItem('cb_role');
  if (_loginRole) state.role = _loginRole;
  var subs = new Set();
  ...
  function nextPatientId() {
    // Use the stored high-water mark so deleted patients never cause ID reuse
    var hwm = state.lastCbId || 2000;
    // Also scan current patients in case state was loaded from an older version
    state.patients.forEach(function (p) {
      var n = parseInt(String(p.id).replace(/\D/g, ""), 10);
      if (!isNaN(n) && n > hwm) hwm = n;
    });
    var next = hwm + 1;
    state.lastCbId = next; // persist the new high-water mark immediately
    return "CB-" + next;
  }

  async function fetchNextPatientIdFromDB() {
    try {
      var sb = window.CB_SB;
      if (!sb) return nextPatientId();
      var res = await sb.rpc("next_cb_patient_id");
      if (res.data) {
        // Keep local high-water mark in sync with whatever the DB returned
        var n = parseInt(String(res.data).replace(/\D/g, ""), 10);
        if (!isNaN(n) && n > (state.lastCbId || 0)) state.lastCbId = n;
        return res.data;
      }
  ...
  window.CBStore = {
    ...
    fetchNextPatientId: fetchNextPatientIdFromDB,
    addPatient: function (input) {
      ...
      // Generate a guaranteed-unique ID — never reuse, never duplicate
      var existingIds = {};
      state.patients.forEach(function (p) { existingIds[p.id] = true; });
      var newId = input._reservedId || nextPatientId();
      while (existingIds[newId]) { newId = nextPatientId(); } // keep going if somehow taken

      var p = {
        id: newId, name: input.name, age: parseInt(input.age, 10) || 0,
    ...
    addCommission: function (input) {
      var c = { id: rid("HC"), hospital: input.hospital || "", patient: input.patient || "", amount: +input.amount || 0, currency: input.currency || "USD", invoiceAmount: input.invoiceAmount != null ? +input.invoiceAmount : 0, originalAmount: input.originalAmount != null ? +input.originalAmount : (+input.amount || 0), commissionRate: input.commissionRate || null, status: input.status || "Unpaid", dueDate: input.dueDate || "", dueISO: input.dueISO || "", recorded: input.recorded || today(), recordedISO: input.recordedISO || "", notes: input.notes || "" };
    ...
    /* ---- agreements ---- */
    getAgreements: function () { return state.agreements || (state.agreements = []); },
    addAgreement: function (input) {
      if (!state.agreements) state.agreements = [];
      var existing = state.agreements.findIndex(function (a) { return a.id === input.id; });
      if (existing >= 0) { state.agreements[existing] = Object.assign({}, state.agreements[existing], input); }
      else { state.agreements.unshift(input); }
      // High-water mark — deleted IDs are permanently retired; new ones always go higher
      var parts = (input.id || "").split("-");
      var seqYear = parseInt(parts[1], 10), seqNum = parseInt(parts[2], 10);
      if (!isNaN(seqYear) && !isNaN(seqNum)) {
        var cur = state.lastAgrSeq;
        if (!cur || cur.year !== seqYear || seqNum > cur.seq) state.lastAgrSeq = { year: seqYear, seq: seqNum };
      }
      log("Agreement created", input.id + " · " + (input.patientName || ""));
      commit();
      return input;
    },
    updateAgreement: function (id, patch) {
      if (!state.agreements) state.agreements = [];
      var idx = state.agreements.findIndex(function (a) { return a.id === id; });
      if (idx >= 0) { state.agreements[idx] = Object.assign({}, state.agreements[idx], patch); log("Agreement updated", id); commit(); }
    },
    deleteAgreement: function (id) {
      if (!state.agreements) return;
      var prev = state.agreements.find(function (a) { return a.id === id; });
      state.agreements = state.agreements.filter(function (a) { return a.id !== id; });
      // lastAgrSeq is intentionally NOT reset — that ID is permanently retired
      log("Agreement deleted", id + " · " + (prev ? prev.patientName || "" : ""));
      commit();
    },
    ...
    _loadFromCloud: function (sbState) {
      ...
      // Role is per-user — never let cloud state overwrite the local user's role
      var localRole = localStorage.getItem('cb_role') || state.role;
      state = sbState;
